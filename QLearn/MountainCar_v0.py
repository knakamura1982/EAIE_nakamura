...
# 行動の種類
class Action(Enum):
    ACCELERATE_TO_LEFT = 0 # 左に加速
    DO_NOT_ACCELERATE = 1 # 加速しない
    ACCELERATE_TO_RIGHT = 2 # 右に加速
    UNDEFINED = 3 # 未定義状態. 念のため設定しておく
ACTION_ID = { Action.ACCELERATE_TO_LEFT:0, Action.DO_NOT_ACCELERATE:1, Action.ACCELERATE_TO_RIGHT:2 } # 行動名から行動番号への変換表
N_ACTIONS = len(ACTION_ID) # 種類数

# 行動名の取得
def get_action_name(action: Action):
    if action == Action.ACCELERATE_TO_LEFT:
        return 'ACCELERATE_TO_LEFT'
    elif action == Action.DO_NOT_ACCELERATE:
        return 'DO_NOT_ACCELERATE'
    elif action == Action.ACCELERATE_TO_RIGHT:
        return 'ACCELERATE_TO_RIGHT'
    else:
        return 'UNDEFINED'
...
def select_action(state, strategy: Strategy):
    ...
    # ランダム戦略
        ...
        return np.random.choice([Action.ACCELERATE_TO_LEFT, Action.DO_NOT_ACCELERATE, Action.ACCELERATE_TO_RIGHT])
...
def main():
    ...
    parser = argparse.ArgumentParser(description='OpenAI Gym MountainCar-v0')
